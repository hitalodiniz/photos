...
interface GalleryDesignFieldsProps {
  ...
  setColumns: (columns: { mobile: number; tablet: number; desktop: number }) => void;
    ...
    <div className="flex flex-wrap items-center gap-x-2.5 gap-y-1.5">
      ...
      <div className="flex items-center gap-2 border-r border-petroleum/10 pr-2.5 shrink-0 h-8">
        ...
        <button
          ...
          style={{ backgroundColor: showBackgroundPhoto ? 'var(--color-gold, #D4AF37)' : '#e2e8f0' }}
        >
          <span className={`absolute top-0.5 left-0.5 w-3.5 h-3.5 rounded-full bg-white shadow-sm transition-transform duration-200 ${showBackgroundPhoto ? 'translate-x-3.5' : ''}`} />
        </button>
      </div>
      ...
      <div className="flex items-center gap-2 shrink-0 h-8">
        ...
        <div className="flex items-center gap-1 h-full">
          {[
            { k: 'mobile' as const, i: Smartphone, label: 'Mob', options: [1, 2, 3, 4] },
            { k: 'tablet' as const, i: Tablet, label: 'Tab', options: [2, 3, 4, 5, 6] },
            { k: 'desktop' as const, i: Monitor, label: 'Desk', options: [3, 4, 5, 6, 8, 10] },
          ].map((d) => (
            <div key={d.k} className="flex items-center gap-1.5 bg-slate-50 border border-petroleum/20 px-1 rounded-[0.25rem] h-5">
              <d.i size={11} className="text-petroleum/60" strokeWidth={2} />
              <div className="relative">
                <select
                  value={columns[d.k]}
                  onChange={(e) => setColumns({ ...columns, [d.k]: Number(e.target.value) })}
                  className="appearance-none bg-transparent pt- pl-0.5 pr-2.5 pb-2 text-[9px] font-bold text-petroleum outline-none cursor-pointer"
                >
                  {d.options.map((v) => (
                ...
                <div className="absolute right-0 top-1/2 -translate-y-1/2 pointer-events-none text-petroleum/60">
                  <svg width="4" height="4" viewBox="0 0 10 10" fill="none">
                    <path d="M1 3L5 7L9 3" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                </div>
              </div>
            </div>

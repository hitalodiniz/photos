...
export function ThumbnailStrip({
  ...
  useEffect(() => {
    if (activeThumbRef.current && containerRef.current) {
      ...
      if (
        thumbLeft < currentScrollLeft ||
        thumbLeft + thumbWidth > scrollRight
      ) {
        container.scrollTo({
    ...
    return (
      <button
        ...
        className={`relative flex-shrink-0 transition-all duration-300 cursor-pointer touch-manipulation ${
          isActive
            ? 'ring-2 ring-champagne ring-offset-2 ring-offset-black scale-110 z-10 shadow-lg shadow-champagne/30'

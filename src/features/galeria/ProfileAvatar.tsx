...
export default function PhotographerAvatar({
  ...
  // 🛡️ Mapeamento de Níveis Progressivos
  ...
  // Classe utilitária para os botões sociais
  const getBtnClass = () =>
      ...
      >
        ...
        >
          <div className="flex flex-col items-start whitespace-nowrap">
            <p
              className={`text-[8px] md:text-[9px] tracking-wider font-medium leading-none mb-0.5 transition-colors duration-300 ${
                position === 'bottom-lightbox'
            ...
            <span
              className={`text-[9px] md:text-[10px] leading-tight transition-colors duration-300 ${
                position === 'bottom-lightbox'
...
Para atender à progressão exata que você definiu, vamos refatorar a lógica de renderização dos ícones. Note que cada plano agora acumula um novo "poder" de conexão social.

PhotographerAvatar Refatorado (Níveis Progressivos)
TypeScript
'use client';
...
export default function PhotographerAvatar({
  ...
  const whatsappLink = `https://wa.me/${photographer.phone_contact?.replace(/\D/g, '')}?text=${encodeURIComponent(GALLERY_MESSAGES.CONTACT_PHOTOGRAPHER_DIRETO())}`;

  // ... (useEffect e useMemo mantidos para performance)

  // Classe utilitária para os botões sociais
  const getBtnClass = () => `p-1.5 rounded-full transition-all border ${
    position === 'bottom-lightbox'
      ? 'bg-black/10 dark:bg-white/10 text-black dark:text-white border-black/20 dark:border-white/20'
      : 'bg-white/10 text-white border-white/10'
  }`;
  ...
  return (
    <div className={`${positionClasses} flex items-center cursor-pointer`} onClick={() => setIsExpanded(!isExpanded)}>
      <div className={`flex items-center rounded-luxury transition-all duration-500 ${isExpanded ? 'gap-2 p-1.5' : 'gap-0 p-0 px-1'} ${/* classes de fundo mantidas */}`}>
        
        {/* AVATAR (Nível: FREE) */}
        <div className="relative w-10 h-10 md:w-14 md:h-14 rounded-full overflow-hidden shrink-0">
          {displayAvatar ? (
            <Image src={displayAvatar} alt={fullName} fill className="object-cover" />
          ) : (
            <div className="w-full h-full bg-slate-800 flex items-center justify-center text-white font-bold">{initialLetter}</div>
          )}
        </div>

        {/* CONTEÚDO EXPANSÍVEL */}
        <div className={`flex flex-col items-start gap-1.5 transition-all duration-500 overflow-hidden ${isExpanded ? 'max-w-[240px] opacity-100 ml-1' : 'max-w-0 opacity-0'}`}>
          <div className="flex flex-col whitespace-nowrap">
            <p className="text-[8px] md:text-[9px] uppercase tracking-tighter text-[#F3E5AB]">Registrado por</p>
            <span className="text-[9px] md:text-[10px] font-bold text-white">{fullName}</span>
          </div>
...
{/* PERFIL INTERNO (Nível: FREE) */}
            ...
            {hasWebsite && photographer?.website && (
              <a href={photographer.website} target="_blank" className={`${getBtnClass()} hover:bg-blue-500`} onClick={e => e.stopPropagation()}>

...
export default function LoadingSpinner({ size = 'md' }: LoadingSpinnerProps) {
  ...
  const sizes = {
    sm: { container: 'w-16 h-16', camera: 'w-6 h-6', blur: 'w-10 h-10' },
    md: { container: 'w-24 h-24', camera: 'w-8 h-8', blur: 'w-16 h-16' },
    lg: { container: 'w-32 h-32', camera: 'w-12 h-12', blur: 'w-20 h-20' },
  };
    ...
    <div className={`relative ${s.container}`}>
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="relative flex items-center justify-center">
          ...
          />
          <Camera
            className="text-[#F3E5AB] relative z-10 w-8 h-8 md:w-10 md:h-10"
            strokeWidth={1}

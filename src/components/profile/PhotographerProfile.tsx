          ...
          <div className="relative w-full max-w-xl">
                ...
                <div className="w-full h-full rounded-full overflow-hidden bg-black flex items-center justify-center p-[2px]">
                  <div className="w-full h-full rounded-full overflow-hidden bg-slate-900 flex items-center justify-center">
                    {photoPreview ? (
                      <Image
                        src={photoPreview}
                        alt={fullName || 'Foto do fotógrafo'}
                        fill // Preenche o container pai
                        sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
                        className="object-cover scale-110" // Mantém seu efeito de zoom
                        priority // Carrega com prioridade por ser parte do Hero/Profile
                      />
                    ) : (
                      <UserIcon
                        size={40}
            ...
            <section className="relative z-10 w-full bg-white rounded-[3rem] p-8 pt-24 shadow-2xl text-center border border-white/20">
              <div className="space-y-6">
                <p
                  className="text-slate-900 text-xl md:text-2xl italic leading-relaxed font-medium px-4"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
                  ...
                  <a
                    href={
                      phone
                        ? `https://wa.me/${phone.replace(/\D/g, '')}?text=${encodeURIComponent(`Olá ${fullName}, vi seu perfil e gostaria de um orçamento!`)}`
                        : '#'
                    }
                    target="_blank"

...
export default function PreferenciasPage({ profile, onClose }) {
    ...
    const [hiddenCats, setHiddenCats] = useState<string[]>(profile?.hidden_system_categories || []);
    const [defaultDesign, setDefaultDesign] = useState(profile?.default_gallery_config || {
        show_cover_in_grid: false,
        grid_bg_color: '#FFFFFF',
        columns: { mobile: 2, tablet: 3, desktop: 4 }
    });
    const [waMessages, setWaMessages] = useState(profile?.whatsapp_messages || {
        gallery_delivery: '',
        lead_contact: ''
    });
                        ...
                        {GALLERY_CATEGORIES.map((cat) => {
                            ...
                            return (
                                <button
                                    ...
                                    type="button"
                                    onClick={() => {
                                        setHiddenCats(prev => isHidden ? prev.filter(id => id !== cat.id) : [...prev, cat.id]);
                                        setHasChanges(true);
                                    }}
                                    className={`flex items-center justify-between p-3 rounded-luxury border transition-all ${isHidden ? 'bg-slate-50 border-petroleum/10 opacity-50' : 'bg-white border-petroleum/30 shadow-sm'
                                        }`}
                                >
                                    <span className="text-[11px] font-bold text-petroleum uppercase tracking-wider">{cat.label}</span>
                                    {isHidden ? <EyeOff size={12} className="text-red-400" /> : <Eye size={12} className="text-gold" />}
                                </button>
                            );
                        })}
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="flex items-center justify-between p-4 bg-slate-50 rounded-luxury border border-petroleum/10">
                            <div className="space-y-1">
                                <p className="text-[10px] font-bold uppercase tracking-widest text-petroleum">foto de fundo ativa</p>
                                <p className="text-[9px] text-petroleum/50 italic">exibir capa como background do grid</p>
                            </div>
                            <button
                                type="button"
                                onClick={() => setDefaultDesign({ ...defaultDesign, show_cover_in_grid: !defaultDesign.show_cover_in_grid })}
                                className={`relative h-5 w-9 rounded-full transition-colors ${defaultDesign.show_cover_in_grid ? 'bg-gold' : 'bg-slate-200'}`}
                            >
                                <span className={`absolute top-0.5 left-0.5 h-4 w-4 rounded-full bg-white shadow-sm transition-transform ${defaultDesign.show_cover_in_grid ? 'translate-x-4' : ''}`} />
                            </button>
                        </div>

                        <div className="flex items-center justify-between p-4 bg-slate-50 rounded-luxury border border-petroleum/10">
                            <p className="text-[10px] font-bold uppercase tracking-widest text-petroleum">cor de fundo do grid</p>
                            <input
                                type="color"
                                value={defaultDesign.grid_bg_color}
                                onChange={(e) => setDefaultDesign({ ...defaultDesign, grid_bg_color: e.target.value })}
                ...
                >
                    <div className="space-y-4">
                        <div className="space-y-2">
                            <label className="text-[10px] font-bold uppercase tracking-widest text-petroleum">mensagem de entrega da galeria</label>
                            <textarea
                                value={waMessages.gallery_delivery}
                                onChange={(e) => setWaMessages({ ...waMessages, gallery_delivery: e.target.value })}
                            ...
                            />
                            <p className="text-[9px] text-gold font-bold italic">use {'{{link}}'} para inserir o link da galeria automaticamente.</p>
                        </div>
                    </div>
                </FormSection>

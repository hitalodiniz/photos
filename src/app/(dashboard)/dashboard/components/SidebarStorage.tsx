...
export default function SidebarStorage({
  ...
  return (
    <div
      className={`pt-6 border-t border-white/5 transition-all duration-500 ${isSidebarCollapsed ? 'px-0' : 'px-2'}`}
    >
      {!isSidebarCollapsed ? (
        <div className="space-y-3">
          ...
          <p className="text-[9px] text-white/20 uppercase tracking-luxury font-semibold px-1">
            Limite de galerias no plano
          </p>
        </div>
      ) : (
        <div className="flex justify-center group relative cursor-help py-2">
          <div className="w-8 h-8 rounded-full border border-white/10 flex items-center justify-center text-[9px] font-semibold text-white/40 hover:text-gold hover:border-gold transition-colors">
            {galeriasCount}
          </div>

...
# SUPABASE_AUTH_REDIRECT_URI_ALLOW_LIST=http://localhost:3000/api/auth/callback,http://localhost:3000/dashboard
# ==============================
EOF
  exit 1
fi
...
export HTTP_PROXY=http://proxy.fazenda.mg.gov.br:8003
export HTTPS_PROXY=http://proxy.fazenda.mg.gov.br:8003
...
AUTH_HEALTH=$(curl --noproxy localhost,127.0.0.1 -s -o /dev/null -w "%{http_code}" http://localhost:54321/auth/v1/health)
...
SETTINGS_JSON=$(curl --noproxy localhost,127.0.0.1 -s http://localhost:54321/auth/v1/settings)
